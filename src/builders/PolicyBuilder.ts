...
function getValueAsEnumOrUndef<T extends object>(row: Record<string, string>, columnName: string, enumType: T): T[keyof T] | undefined {
...
export function fromCSVRow(row: Record<string, string>): Policy {
  ...
  const policy: Policy = {
    ...
    state: getValueAsEnumOrUndef(row, col("state"), ConditionalAccessPolicyState),
    conditions: {
      applications: {
        ...
        applicationFilter: {
          mode: getValueAsEnumOrUndef(row, col("conditions.applications.applicationFilter.mode"), FilterModeType),
      ...
      clientApplications: {
        ...
        servicePrincipalFilter: {
          mode: getValueAsEnumOrUndef(row, col("conditions.clientApplications.servicePrincipalFilter.mode"), FilterModeType),
      ...
      devices: {
        deviceFilter: {
          mode: getValueAsEnumOrUndef(row, col("conditions.devices.deviceFilter.mode"), FilterModeType),
    ...
    grantControls: {
      ...
      operator: getValueAsEnumOrUndef(row, col("grantControls.operator"), Operator),
    ...
    sessionControls: {
      ...
      cloudAppSecurity: getValueAsEnumOrUndef(row, col("sessionControls.cloudAppSecurity.cloudAppSecurityType"), CloudAppSecurityType),
      continuousAccessEvaluation: getValueAsEnumOrUndef(row, col("sessionControls.continuousAccessEvaluation.mode"), ContinuousAccessEvaluationType),
      disableResilienceDefaults: boolOrUndef(row, col("sessionControls.disableResilienceDefaults")),
      persistentBrowser: getValueAsEnumOrUndef(row, col("sessionControls.persistentBrowser.mode"), PersistentBrowserMode),
      ...
      signInFrequency: {
        ...
        type: getValueAsEnumOrUndef(row, col("sessionControls.signInFrequency.type"), SignInFrequencyType),
        authenticationType: getValueAsEnumOrUndef(row, col("sessionControls.signInFrequency.authenticationType"), SignInFrequencyAuthenticationType),
        frequencyInterval: getValueAsEnumOrUndef(row, col("sessionControls.signInFrequency.frequencyInterval"), SignInFrequencyInterval),
      },
    },
  }
  return policy
